...
public:
    ...
    vector3d<T> vector() const
    {
        ...
        //vectors have 0 at end
        //vectors3d(const std::string& name, int dims, const std::initializer_list<T>& li
        return vector3d<T>("Vect", 3, {x, y, z, 0});
    ...
    matrix3d<T> rot_matrix() const {
        //return matrix3d<T>("Rotation", 3, (-2.0 * pow(y,2) + pow(z,2))+1, 2 * (x*y - w*z), 2 * (x*z)
        vector3d<T> vec1("Rotation", 3, { -2.0*(pow(y, 2) + pow(z, 2)) + 1, 2.0*(x*y + w*z), 2.0*(x*z - w*y)});
        vector3d<T> vec2("Rotation", 3, {  2.0*(x*y - w*z), -2.0*(pow(x, 2) + pow(z, 2)) + 1, 2.0*(y*z + w*x) });
        vector3d<T> vec3("Rotation", 3, {  2.0*(x*z + w*y) , 2.0*(y*z - w*x), -2.0*(pow(x, 2) + pow(y, 2)) + 1  });
        return matrix3d<T>("Matrix Rotation", 3, { vec1, vec2, vec3 });
    }
